...
class CustomTextDataset(Dataset):
    ...
    def __init__(self, df):
        self.df = df
        self.vocab_size = RobertaTokenizer.from_pretrained('roberta-base').vocab_size
    def __getitem__(self, idx):
        ...
        # 检查 input_ids 范围 
        if any(id >= self.vocab_size or id < 0 for id in item['input_ids']): 
            print(f"Error: input_ids out of range at index {idx}, input_ids: {item['input_ids']}") 
            return item
        return item
...
def save_checkpoint(model, optimizer, epoch, batch_idx, file_path):
    checkpoint = {
        'epoch': epoch,
        'batch_idx': batch_idx,
        'model_state_dict': model.state_dict(),
...
def load_checkpoint(model, optimizer, file_path):
    if os.path.isfile(file_path):
        ...
        print(f"Checkpoint loaded from {file_path}, resuming training from epoch {epoch}, batch {batch_idx}")
        return epoch, batch_idx
    else:
        print(f"No checkpoint found at {file_path}, starting training from scratch")
        return 0, 0

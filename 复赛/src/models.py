...
class BERTClass(torch.nn.Module):
    def __init__(self, args):
        ...
        self.l1 = transformers.AutoModel.from_pretrained("/root/.cache/huggingface/transformers/roberta-base", ignore_mismatched_sizes=True).to(args.device)
    ...
    def forward(self, ids, mask, features):
        ...
        assert ids.shape == mask.shape, f"Shape mismatch: ids.shape={ids.shape}, mask.shape={mask.shape}" 
        assert ids.max().item() < 50265, f"索引超出范围: {ids.max().item()} >= 50265" 
        assert ids.min().item() >= 0, f"索引超出范围: {ids.min().item()} < 0"

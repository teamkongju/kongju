...
from utils import set_random_seed, CustomTextDataset, loss_fn, create_modified_dataset, save, load, get_free_gpus,save_checkpoint,load_checkpoint
...
def train(args):
    ...
    '''
    checkpoint_path = os.path.join(args.SAVED_MODELS_DIR, args.MODEL_NAME + "_checkpoint.pth") 
    start_epoch, start_batch = load_checkpoint(model, optim, checkpoint_path)
    print(f"[INFO] Start from batch {start_batch}, Epoch {start_epoch+1}")
    '''
        ...
        if should_train:
            for idx,batch in enumerate(tqdm(train_loader)):
                ...
                if idx % args.update_freq == 0 and idx != 0:
                    #save_checkpoint(model, optim, epoch, idx, checkpoint_path) 
...
if __name__ == "__main__":
    ...
    wandb.init(entity="zyanno1-001", project="DaSE Final")
    train(args)